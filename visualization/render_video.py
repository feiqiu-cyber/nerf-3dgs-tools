...
def render_video(model_path, iteration, views, gaussians, pipeline, background):
    ...
    for idx, pose in enumerate(tqdm(generate_ellipse_path(views,n_frames=600), desc="Rendering progress")):
        ...
        rendering = render(view, gaussians, pipeline, background)["render"].clamp(0.0, 1.0).to("cpu")
        rendering = rendering.detach().permute(1, 2, 0).numpy()
        rendering = cv2.cvtColor(np.asarray(rendering), cv2.COLOR_RGB2BGR)
        # torchvision.utils.save_image(rendering, os.path.join(render_path, '{0:05d}'.format(idx) + ".png")
        if not 'out' in locals().keys():
            fourcc = cv2.VideoWriter.fourcc('m', 'p', '4', 'v')
            mp4_name = model_path + '/video.mp4'
            out = cv2.VideoWriter(mp4_name, fourcc, fps=60,
                                frameSize=(rendering.shape[1], rendering.shape[0]), isColor=True)
...
def render_fly(dir_full_render, dataset, iteration, pipeline,  args,
               on_the_fly_camera_path, mp4_postfix):
    print("------ render on the fly -------")
    with torch.no_grad():
        gaussians = GaussianModel(dataset.sh_degree)
        scene = Scene(dataset, gaussians, load_iteration=iteration, shuffle=False, args_all=args)

        bg_color = [1, 1, 1] if dataset.white_background else [0, 0, 0]
        background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")

        # view0 = scene.getTrainCameras()[0]
        views = scene.get_on_the_fly_Cameras(args,on_the_fly_camera_path)

        if 1:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                total_start = time.time()
                for idx, view in enumerate(tqdm(views, desc="on the fly progress")):
                    rendering = render(view.to("cuda"), gaussians, pipeline, background)["render"].clamp(0.0, 1.0).to("cpu")
                    rendering = rendering.detach().permute(1, 2, 0).numpy()
                    rendering = cv2.cvtColor(np.asarray(rendering), cv2.COLOR_RGB2BGR)

                    if not 'out' in locals().keys():
                        fourcc = cv2.VideoWriter.fourcc('m', 'p', '4', 'v')
                        mp4_name = dataset.model_path + "/" + dataset.model_path.split('/')[-1] + mp4_postfix + '.mp4'
                        out = cv2.VideoWriter(mp4_name, fourcc, fps=60,
                                              frameSize=(rendering.shape[1], rendering.shape[0]), isColor=True)
                ...
                total_end = time.time()
                print(f" render full check video : {total_end - total_start}s")
